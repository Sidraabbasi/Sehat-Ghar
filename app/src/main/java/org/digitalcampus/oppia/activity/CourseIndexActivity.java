...
public class CourseIndexActivity extends AppActivity implements OnSharedPreferenceChangeListener, ParseCourseXMLTask.OnParseXmlListener,  GestureDetector.OnGestureListener, GestureDetector.OnDoubleTapListener{
    ...
    NavigationBarStatus mNavigationStatusListener ;
    ...
    ArrayList<String> courseVideoList;
    private String digestJumpTo;
    CardView actionPoint;
    WebView headerImage ;
    CourseVideoLayoutAdapter courseVideoLayoutAdapter;
    @Inject  CompleteCourseProvider completeCourseProvider;
    GestureDetector mGestureDetector;
    ArrayList<Activity> actionPointList = new ArrayList<>();
    ArrayList<Course> courseForActionPoints = new ArrayList<>();
    WebView topicswebView;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        initializeDagger();
        ((TextView) findViewById(R.id.descriptionLabel)).setText(Html.fromHtml(getString(R.string.description_course)));
        prefs = PreferenceManager.getDefaultSharedPreferences(this);
        prefs.registerOnSharedPreferenceChangeListener(this);
        actionPoint = (CardView) findViewById(R.id.actionPointsCV);
        loadingCourseView =  findViewById(R.id.loading_course);
        mGestureDetector = new GestureDetector(this, this);
        recyclerView = (RecyclerView) findViewById(R.id.videoList);
        mNavigationStatusListener = (OppiaMainActivity)OppiaMainActivity.getContext();
        headerImage  = (WebView) findViewById(R.id.headerImage);
        topicswebView  = (WebView) findViewById(R.id.topicsView);

        courseVideoList = new ArrayList<>();
        actionPoint.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {

                Intent intent = new Intent(CourseIndexActivity.this, ActionPointsListActivity.class);
                intent.putExtra("actionPoints", actionPointList);
    ...
    public void onParseComplete(CompleteCourse parsed) {
        ...
        WebView wv0 = (WebView)findViewById(R.id.fragment_webview);
        //wv0.getSettings();
        wv0.setBackgroundColor(Color.TRANSPARENT);

       // wv0.setLayerType(View.LAYER_TYPE_SOFTWARE, null);
        //WebView wv1 = (WebView)findViewById(R.id.fragment_webview1);
        //WebView wv2 = (WebView)findViewById(R.id.fragment_webview2);
        ...
        for (int i=0;i<sections.size();i++)
        {
            ...
            if (i ==0)  {
                ...
                loadView(headerImage,section.getActivities().get(0));
                loadView(wv0,section.getActivities().get(1));
            }
            for(int j=0;j<section.getActivities().size();j++){
                final Activity activity = section.getActivities().get(j);
                if (activity.getSectionId() == 2 )
                {
                    if (!activity.getMultiLangInfo().getTitle("en").equals("موضوع")){
                        videoList.add(activity);
                     }
                     else {
                ...
                else if (activity.getSectionId() == 1) {

                }
                else {
                    actionPointList.add(activity);
                }
            }
        }

        loadView(topicswebView,topic);
        courseVideoLayoutAdapter = new CourseVideoLayoutAdapter(this,videoList, course );
        LinearLayoutManager layoutManager
                = new LinearLayoutManager(this, LinearLayoutManager.HORIZONTAL, false);
        recyclerView.setLayoutManager(layoutManager);
        recyclerView.setAdapter(courseVideoLayoutAdapter);
    }

    public void loadView(WebView wv, final Activity activity){
        ...
        try {
            ...
            wv.addJavascriptInterface(
                    new JSInterfaceForResourceImages(CourseIndexActivity.this, course.getLocation()),
        ...
        wv.setWebViewClient(new WebViewClient() {
            ...
            public boolean shouldOverrideUrlLoading(WebView view, String url) {
                ...
                if (url.contains("/video/")) {
                    ...
                    CourseIndexActivity.this.startMediaPlayerWithFile(mediaFileName,activity);
                    return true;
    ...
    protected void startMediaPlayerWithFile(String mediaFileName, Activity activity){
        ...
        boolean exists = Storage.mediaFileExists(CourseIndexActivity.this, mediaFileName);
        if (!exists) {
            Toast.makeText(CourseIndexActivity.this,
                    CourseIndexActivity.this.getString(R.string.error_media_not_found, mediaFileName),
                    Toast.LENGTH_LONG).show();
            return;
        }

        String mimeType = FileUtils.getMimeType(Storage.getMediaPath(CourseIndexActivity.this) + mediaFileName);
        ...
        Intent intent = new Intent(CourseIndexActivity.this, VideoPlayerActivity.class);
        Bundle tb = new Bundle();
    ...
    public boolean onDoubleTap(MotionEvent e) {
        mNavigationStatusListener.showHide();
        return true;
    }
